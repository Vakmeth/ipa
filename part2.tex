...
Die Ergebnisse der IPA werden in Kurzform beschrieben.
...
und Organisationen verwendet und befindet sich darum in einer kontinuierlichen Weiterentwicklung. Mit dem Wagons-Gem
...
Ich selbst arbeite jetzt seit einem halben Jahr im Hitobito und nahm darin vor allem Upgrades und Migrationen vor. So durfte der Kandidat
bspw. das Upgrade von RoR (Ruby on Rails) von 6.1 auf 7.1 vornehmen oder die Migration von MySQL auf PostgreSQL vollziehen.
...
welche im Tool etabliert wurden. Einer dieser komplexen Abläufe, ist die Filterung nach Personen oder Abonnennten.

Mit dieser IPA eine neue Benutzerschnittstelle entworfen werden eine besser User Experience für den Benutzer zu schaffen.
...
Personen bedient wird, welche nicht das technische Know-How dahinter besitzen, ist es wichtig, Arbeitsabläufe so einfach wie möglich zu entwerfen, um 
...
\section{Ist-Zustand}
Im folgenden Abschnitt wird erklärt, wie die Filterung von Personen und Abonnenten im Hitobito zum
...
Unter dem Dropdown ``Weitere Ansichten'' kann der Benutzer entweder einen bestehenden Personenfilter auf die angezeigte Personenliste anwenden 
...
Die drei Radio-Buttons am Anfang, definieren auf welcher Ebene gesucht wird.
...
kann er dies mit einem Klick auf den Namen einer Ebene tun. In diesem Beispiel wäre das ein Klick auf ``Hauptebene''. Nach dem Klick auf den
Ebenennamen werden alle Checkboxen abgehakt. Somit wurden alle Rollen der angeklickten Eben ausgewählt.

Des weiteren kann für die Rollen ein Gültigkeits-Zeitraum definiert werden. Die Gültigkeit
wird vom Benutzer via Radio-Buttons ausgewählt. Zuletzt kann im ``Rollen'' Dropdown entschieden werden, ob 
archivierte Rollen bei der Filterung berücksichtigt werden. Das UI bietet dem Benutzer dafür eine weitere selbststehende
Checkbox.
...
des Filterkriteriums definiert werden.
...
Es können auch mehrere Sprachen ausgewählt werden. Die Auswahl erfolgt über Checkboxen.
...
\subsection{Abonnente}
Die Abonnentenfilterung kann im Hitobito nicht wie die Personenfilterung angezeigt werden. Stattdessen
wird die Filterung bei einem Export der Abonnenten als CSV angewendet. So werden im resultierenden CSV nur Personen
angezeigt, welche den Filterkriterien entsprechen.

Die Abonnentenfilterung kann erneut über die Gruppenübersicht ausgemacht werden.
...
Daraufhin wird der Benutzer auf die Infoseite des Abonnements weitergeleitet. Um die Abonnentenfilterung
einzusehen, muss nun auf den Reiter ``Abonnenten'' navigiert, werden.

\begin{figure}[h]
   \centering
   \fbox{\includegraphics[width=1\textwidth,]{single_subscription_overview.png}}
   \caption{Übersicht einzelnes Abonnementes}
\end{figure}

In dieser Ansicht ist es dem Benutzer möglich, diverse Filterkriterien für die Abonnenten zu definieren.
...
\begin{figure}[h]
   ...
   \caption{Abonnentenübersicht}
\end{figure}

\newpage

In den globalen Bedingungen bieten sich zwei Filterkriterien an: Felder und Sprache. Die Funktionsweise
...
werden in der Textbox unter dem Titel ``Globale Bedingungen'' auf der Abonnentenübersicht angezeigt.
\begin{figure}[h]
   \centering
   \fbox{\includegraphics[width=1\textwidth,]{global_conditions_overview.png}}
   \caption{Globale Filterungsbedingungen für Abonnenten}
\end{figure}
...
Die Übersicht enthält die Radio-Buttons der alten Benutzerschnittstelle. Die massgebliche Änderung zum Soll-Zustand, betrifft den Hinzufügen-Button.
Dieser agiert als Dropdown, sobald der Benutzer auf ihn klickt. Das Dropdown besitzt vier Filterungskriterien, nicht mehr die fünf, wie sie momentan
...
Die definierten Filterkriterien verschwinden aus dem Dropdown unter dem Hinzufügen-Button. Stattdessen können 
die definierten Filterkriterien über den Stift in der oberen, rechten Ecke einer Box, bearbeitete werden.
...
somit aus dem Dropdown. Wurden zu jedem Filterkriterium Bedingungen erfasst, verschwindet der gesamte Hinzufügen-Button.
...
ein Filterkriterium entweder per Hinzufügen-Button hinzufügt oder per Editier-Button, Stift in der oberen rechten Ecke der Box, bearbeitet.

\textbf{Felder}

Die Felder können im neuen Mockup durch ein Dropdown ausgewählt werden. Danach kann wie zuvor die Genauigkeit bestimmt werden.
Will der Benutzer ein weiteres Attribut zu seinen Suchbedingungen hinzufügen, kann er dies über den Hinzufügen-Button in der rechten unteren Ecke. Auf dieser
...
Das Element des Stichdatums bleibt bestehen. Die Checkbox für die archivierten Rollen wird im Mockup vernachlässigt und alle Rollen werden standardmässig, sofern
diese die Gültigkeitsbedingungen erfüllen, miteinbezogen.
...
Als unbekannte Informationen gelten: Die Bedürfnisse des Benutzers, Sicherheitsrisiken, sowie die daraus resultierenden Produktanforderungen.
Die genannten Bereiche werden in den kommenden Abschnitten aufgenommen und analysiert. Alle unbekannten Informationen, welche einzelne 
...
\section{Änderungen am Mockup}
Während des Implementierens wurden Aspekte entdeckt, welche am Mockup geändert werden mussten.
Diese werden im folgenden Abschnitt dokumentiert.

\begin{itemize}
   \item \textbf{Bearbeitung der Filterkriterien: } Um Filterkriterien zu bearbeiten muss nicht auf ein Bearbeitungssymbol geklickt werden. Die Bearbeitung kann direkt nach dem Hinzufügen eines Filterkriteriums vorgenommen werden
   \item \textbf{Gruppendropdown im Rollenfilterkriterium: } Es wird kein Dropdown zur Filterung der Rollen nach Gruppen zur Verfügung gestellt. Die Rollen sind alle über ein einzelnes Dropdown auswählbar.
   \item \textbf{Entfernung eines Filterkriteriums: } Als zusätzliche Funktionalität wird ein Mülleimer Symbol auf jedem Filterkriterium eingefügt, um dieses von der Filterkonfiguration zu entfernen.
   \item \textbf{Filterkriterium wird beibehalten: } Die Benutzerschnittstelle für das Bearbeiten des Filterkriteriums ``Felder'' wird nicht modifiziert und lediglich in der neuen Benutzerschnittstelle angeordnet. 
\end{itemize}
...
\begin{table}[h!]
   \begin{tabular}{|L{0.2\textwidth}|L{0.5\textwidth}|L{0.2\textwidth}|}
       ...
       \hline
       Umfragen & Hierfür wird eine grosse Benutzergruppe benötigt, um ein aussagekräftiges Resultat daraus zu ziehen. 
       ...
       In einem Testskript muss jeder Schritt und die daraus folgenden Aufgabe klar definiert sein. Dies geht
...
\begin{itemize}
   \item Frage 7: Welche Erfahrungen hast du mit der Filterung von Personen und Abonnenten in Hitobito gemacht?
   \item Frage 8: Was stört dich an dieser Filterung?
   \item Frage 9: Was würdest du an dieser Filterung ändern?
   ...
   \item Frage 12: Welche Dringlichkeit hat eine Überarbeitung der Filterung für dich?
   \item Frage 13: Welche Kosten wärst du maximal bereit zu zahlen?
\end{itemize}

\newpage

\subsection{Ablaufprotokoll}
\begin{table}[h!]
   \begin{tabular}{|L{0.2\textwidth}|L{0.4\textwidth}|L{0.3\textwidth}|}
       ...
       \hline
       Erklärung des Ablaufs & Perfekt, ja das stimmt so für mich. & Testperson richtet den Blick aufmerksam auf den Befrager. \\
       \hline
       Frage 1 & Ich bin teilweise Product Owner von Hitobito. Eigentlich ist Oliver Dietschi der Product Owner, aber ich höre von vielen Kunden, was ihre Bedürfnisse sind. & Testperson Lächelt leicht. \\
       \hline
       Frage 2 & Im Moment ist mir Nachhaltigkeit unheimlich wichtig. In allen Bereichen, Beziehungen und Freundschaften. Auch was ich kaufe und wie ich lebe. & Testperson wird ernster. Stimme senkt sich. \\
       \hline
       ...
       Wirtschaftsstudium an der BFH gemacht. Dort habe ich gemerkt, dass mir Projektmanagement sehr zusagt. Anschliessend habe ich im Bereich Ausbildung und in der Stahlindustrie als Projektmanager gearbeitet.
...
\begin{table}[h!]
   \begin{tabular}{|L{0.3\textwidth}|L{0.4\textwidth}|L{0.2\textwidth}|}
       ...
       \rowcolor{puzzleblue}\color{white}Tätigkeit & \color{white} Antwort der Testperson & \color{white} Mimik und Gestik \\
       \hline
       Frage 4 & Ich wusste zuerst noch gar nicht, wie Hitobito funktioniert. Ein Kollege von mir, Tobi Hinderling, hatte
       dann von einer Firma gehört, welche Hitobito betreibt. Er sagte sie sei super cool organisiert und er wolle unbedingt dort arbeiten. Das machte er
       dann auch und hatte das Projektmanagement bei Hitobito inne. Tobias und ich, wir waren immer wieder am ``Aare böttle'' zusammen und da hat er mir gesagt, ich soll doch auch zu Puzzle kommen,
       denn ihn würde mehr das UX als das Projektmanagement interessieren. Wir könnten dann intern die Rollen wechseln und so ist es geschehen. & Testperson Lacht bei der Anektote zu Tobias Hinderling. \\
       \hline
       Frage 5 & Die Kunden faszinieren mich. Wir haben eine breite Kundenpalette. Wir streben auch eine langfristige Kunden an und nehmen nicht jeden.
       ...
       \hline
       Frage 7 & Ich habe die Filterung schon verwendet aber meist im Umfang von Tests und Abnahme der Akzeptanzkriterien von Tickets. & Testperson nickt als Bestätigung. \\
     \hline
     \end{tabular}
...
\begin{table}[h!]
   \begin{tabular}{|L{0.3\textwidth}|L{0.4\textwidth}|L{0.2\textwidth}|}
       ...
       \hline
       Frage 9 & Ich würde versuchen, alles zu vereinheitlichen. Evtl. die Felder und Sprache zusammennehmen. Auch die Rollen
...
\begin{table}[h!]
   \begin{tabular}{|L{0.3\textwidth}|L{0.4\textwidth}|L{0.2\textwidth}|}
       ...
       \rowcolor{puzzleblue}\color{white}Tätigkeit & \color{white} Antwort der Testperson & \color{white} Mimik und Gestik \\
       \hline
       Frage 11 & Als erstes das Sprachen Dropdown aus der Landschaft entfernen und unter Felder anordnen. Der nervt mich tatsächlich sehr. Sowieso haben wir für jedes
       Attribut ein Dropdown in den Feldern, aber für die Sprachen hängt es hier einfach so herum? Ansonsten finde
       ich alles andere ok. Geschlecht ist etwas anderes was mich stört. Als Mann musst du dort ein ``m'' eingeben und als Frau ein ``w''. 
       Auch ``w''' wenn du die Sprache auf französisch eingestellt hast. Und weisst du was? Wenn du kein Geschlecht hast, musst du das Feld leer lassen, was einfach
       niemand weiss. & Testperson regt sich auf, Gestikuliert in Richtung Hitobito Filterung auf Laptop. \\
       \hline
       Frage 12 & Da kommt bei mir der Ökonom durch. Alles ist nice to have. Wir haben in Hitobito im Moment noch andere Bausteine, welche ich noch dringender priorisiere. Die Filterung
...
\begin{table}[h!]
   \begin{tabular}{|L{0.3\textwidth}|L{0.4\textwidth}|L{0.2\textwidth}|}
       ...
       entwickeln das Produkt dann einfach im Auftrag von Kunden weiter.
         & Testperson lacht zu Beginn seiner Antwort. Lachen mündet in Lächeln und verschwindet gegen Ende der Antwort. Person gestikuliert mittelmässig. \\
       \hline
       Verabschiedung & Vielen Dank dir. Wenn du Fragen oder noch weiteres wissen musst, kannst du gerne nochmals zu mir kommen. & Testperson lächelt und verabschiedet sich mit einem kurzem Wink. \\
     \hline
     \end{tabular}
...
\begin{table}[h!]
   \begin{tabular}{|L{0.2\textwidth}|L{0.7\textwidth}|}
      ...
      \hline
       D2 & Mittlere Dringlichkeit. Bedürfnis muss in den nächsten Monaten umgesetzt werden. Nicht Erfüllung führt zu überschaubarem Nutzerverlust. \\
      \hline
       D3 & Niedrige Dringlichkeit. Bedürfnis muss im nächsten Jahr umgesetzt werden. Nicht Erfüllung führt zu keinem Nutzerverlust. \\
     \hline
     \end{tabular}
...
\begin{table}[h!]
   \begin{tabular}{|L{0.2\textwidth}|L{0.3\textwidth}|L{0.1\textwidth}|L{0.3\textwidth}|}
       ...
       stehen. Die Filterung soll einem modularen Aufbau folgen, welcher den User durch die Filterung führt. & D1 & Durch den unübersichtlichen Aufbau der Filterung und die Reaktion der Testperson, wird diesem Bedürfnis die höchste Priorität zugeschrieben. \\
       \hline
...
\begin{table}[h!]
   \begin{tabular}{|L{0.3\textwidth}|L{0.6\textwidth}|}
       ...
       \hline
       Beschreibung & Es werden im Rahmen der Implementation etablierte Libraries. Basiert auf \hyperref[sec5]{\color{blue}Sicherheitsrisiko 5}  \\
       \hline
       Messbarkeit & Die verwendeten Libraries sind bekannt und besitzen eine Vielzahl von Downloads  \\
       \hline
     \end{tabular}
...
\begin{table}[h!]
   \begin{tabular}{|L{0.3\textwidth}|L{0.6\textwidth}|}
       ...
       \hline
       Beschreibung & Filterung soll ausschliesslich verifizierte Daten anzeigen, basiert auf \color{blue}\hyperref[sec1]{\color{blue} Sicherheitsrisiko 1}\color{black}  \\
       \hline
...
\begin{table}[h!]
   \begin{tabular}{|L{0.3\textwidth}|L{0.6\textwidth}|}
       ...
       weitere Filterkriterien dazukommen können. \\
       \hline
...
\begin{table}[h!]
   \begin{tabular}{|L{0.3\textwidth}|L{0.6\textwidth}|}
       ...
       \hline
       Beschreibung & Neue Filterkriterien können mit dem Hinzufügen-Button hinzugefügt werden. \\
       \hline
...
und der Benutzerschnittstelle der globalen Bedingungen der Abonnenten. Andere Benutzerschnittstellen, welche mit der Filterung verbunden sind,
werden erst nach der IPA überarbeitet.

Während dieser IPA wird die Funktionalität nur mit dem Hitobito und Hitobito-Generic Wagon garantiert. Alle anderen Wagons 
...
\chapter{Entwurf}
Es werden Lösungskonzepte für den erstellten Anforderungskatalog erarbeitet.
...
Aus dem Anwendungsdiagramm werden die 4 Anwendungsfälle entnommen und hier im Detail beschrieben. Vier Anwendungsfälle, da die Anzeige,
...
\begin{table}[h!]
   \begin{tabular}{|L{0.3\textwidth}|L{0.6\textwidth}|}
       ...
       Vorbedingungen & 
       \begin{itemize}
         \item Der Benutzer besitzt die nötigen Rechte um einen Filter zu erstellen
...
\begin{table}[h!]
   \begin{tabular}{|L{0.3\textwidth}|L{0.6\textwidth}|}
      ...
      \hline
      Kurzbeschreibung & Der Benutzer kann die Filterkriterien bearbeiten. \\
      \hline
      Vorbedingungen & 
      \begin{itemize}
         \item Der Benutzer hat einen Filter gespeichert
...
Anschliessend werden mögliche Lösungsvarianten konzeptioniert. Aus den Lösungsvarianten wird per Variantenentscheid eine Lösungsvariante ausgearbeitet.
...
Die Rails Applikation verwaltet die Geschäftslogik von Hitobito. Die Erweiterungen dieser Arbeit
...
Turbo angesteuert werden können. Hierbei sollen nur Turbostreams verwendet werden. Diese ermöglichen es, Elemente im HAML mit der ID 
...
Rot markierte Felder stehen dabei für Funktionen, welche entfernt werden. Grün steht für Funktionen, welche hinzugefügt werden.
...
von diesem entfernt. Da die Filterkriterien mittels der Turbostreams angezeigt werden, wird der Endpoint ``new'' im \texttt{PeopleFilterController}
nicht mehr benötig. Die View \texttt{\_filter.html.haml} muss so geändert werden, dass sie alle Boxen, welche durch die Filterkriterien
auf der Benutzerschnittstelle darstellen, in ein \texttt{Div} verpacken. Dieses \texttt{Div} kann später in den 
...
Turboframe. Wird im Dropdown auf eines der Filterkriterien geklickt, wird der Inhalt des Turboframes mit dem Formular für das jeweilige 
...
Massgebend ist der Unterschied, dass hier keine Turbostreams benötigt werden. Es wird nach wie vor für jedes Filterkriterium
ein Endpoint erstellt. Dieser liefert allerdings die bereits bestehenden Partials zurück. An den Partials selbst muss ein Turboframe eingebaut werden.
So wird jedes Formular eines Filterkriteriums von einem Turboframe umschlossen. Als zusätzliche Klasse entsteht in dieser Variante der \texttt{FilterDropdownController}.
...
Im folgenden Abschnitt werden den definierten Kriterien Gewichtungen hinzugefügt, da nicht jedes Kriterium gleich wichtig für
diese IPA ist.
...
\begin{itemize}
   ...
   \item \textbf{Einführung 20\%:} Es ist wichtig eine Lösung zu implementieren, welche schnell ihren Weg in die produktive Umgebung findet, da die Einführung von der IPA ausgenommen ist, wird diesem Kriterium eine geringere Gewichtung zugeordnet.
   ...
   \item \textbf{Performanz 10\%: } Ist eine Applikation zu langsam und benötigt mehrere Minuten bis sie Resultate geladen hat, kann das den Benutzer schnell vor den Kopf stossen und dazu führen, dass dieser die Applikation in Zukunft nicht mehr verwendet.
\end{itemize}
...
\begin{table}[h!]
   \begin{tabular}{|L{0.2\textwidth}|L{0.2\textwidth}|L{0.6\textwidth}|}
      ...
      \hline
      Performance & 8 & Performance wird optimiert. \\
      \hline
   \end{tabular}
...
Da in dieser Erweiterung Benutzereingaben an das Rails-Backend gesendet werde,n muss der Cross-Site Scripting Angriff ebenfalls
berücksichtig werden. Rails selbst bietet dafür einen eingebauten Abwehrmechanismus. Ein solcher XSS-Angriff kann folgendermassen aussehen:
\newline
...
dass der Absender der Anfrage die nötigen Berechtigungen für das Einsehen der Informationen hat. Die Prüfung der Berechtigungen sieht wie folgt
aus: 
...
\section{Testkonzept}
Das Testkonzept dient der Planung durchzuführender Tests.
...
an diesen Tests vorgenommen. Die bestehenden Tests werden aus zeitlichen Gründen bei Fehlschlagen auskommentiert und ignoriert. Im Rahmen dieser Arbeit, sind
auschliesslich die selbstverfassten Tests relevant.
...
\begin{table}[h!]
   ...
   \begin{tabular}{|L{0.4\textwidth}|L{0.6\textwidth}|}
       ...
       Erwartetes Resultat & 
       \begin{itemize}
         \item Box des Filterkriteriums ``Felder'' wird mit den eingegebenen Informationen ergänzt.
      \end{itemize} \\
     \hline 
     \end{tabular}
     \caption{Testfall 5}
...
\begin{table}[h!]
   ...
   \begin{tabular}{|L{0.4\textwidth}|L{0.6\textwidth}|}
       ...
       \begin{itemize}
         ...
         \item Hinzufügen-Button in der unteren rechten Ecke klicken
...
In den Testfällen werden teils auf Testdaten verwiesen, welche hier dokumentiert sind:
...
\begin{table}[h!]
   ...
   \begin{tabular}{|L{0.4\textwidth}|L{0.6\textwidth}|}
       ...
       Erwartetes Resultat & 
       \begin{itemize}
         \item Alle Personen in der resultierenden Personenliste weisen die gefilterten Felder auf
       \end{itemize} \\
     \hline
     \end{tabular}
     \caption{Testfall 11}
...
\begin{table}[h!]
   ...
   \begin{tabular}{|L{0.4\textwidth}|L{0.6\textwidth}|}
       ...
       Erwartetes Resultat & 
       \begin{itemize}
         \item Export der Abonnenten als CSV beinhaltet alle Personen, welche die globalen Bedingungen erfüllen.
       \end{itemize} \\
     \hline
     \end{tabular}
     \caption{Testfall 14}
...
Da diese Anforderungen nicht durch Features oder manuelle Tests abgedeckt werden können, werden diese Anforderungen
...
Grund dafür ist, dass die Abschnitte ``Schutzbedarfsanalyse'', ``Einführung'' und ``Risikoanalyse für Projektrisiken'' in der Dokumentation noch nicht beschrieben wurden. 
...
Es werden zwei Sprint Diagramme analysiert.
...
\chapter{Umsetzung}
Die Umsetzung behandelt technische Details zur Implementation der Lösungsvariante, sowie 
die Aufnahme der Abweichungen von dieser.
...
Während des Entwickelns, traten auch beim Hinzufügen-Button Änderungen am Konzept auf. Das Hauptproblem war, dass 
der Button, nachdem alle Filterkriterien hinzugefügt wurden, nicht mit den Turbostreams entfernt werden konnte. 
...
In diesem Fall wird ein MutationsObserver registriert, welcher prüft ob das registrierte Element
...
ursprünglich für jedes Filterkriterium ein Turbostream angedacht. Durch die Verwendung nur einer Schnittstelle, wurde entsprechend nur ein generischer Turbostream für 
...
Durch die Sicherstellung des korrekten Formates, kann das Filterkriterium ``Felder'' erneut vom Controller interpertiert und 
gespeichert werden.

\section{Globale Bedingungen}
Die globalen Bedingungen der Abonnenten und der Personenfilter, verwenden die gleichen Benutzerschnittstellen für das Filterkriterium ``Felder''.
Durch die Anpassung der Benutzerschnittstelle des Filterkriteriums, wurden die globalen Bedingungen indirekt auch angepasst.
...
Anders als beim Personenfilter, ist es nicht möglich, das Filterkriterium zu entfernen.
...
wird das Symbol ausgeblendet. Im Personenfilter ist die Variable stets auf ``true'' gesetzt.
...
\textbf{Globale Bedinungen der Abonnenten}

In Hitobito ist es möglich globale Bedingungen für Abonnenten zu definieren. Personen welche diese Bedinungen erfüllen, 
...
\chapter{Einführung}
Im folgenden Abschnitt wird behandelt, wie das IPA Feature in die bestehende Hitobito Applikation eingeführt werden wird und
welche weitere Arbeiten vor diesem Schritt erledigt werden müssen.

\section{Einführung des Systemes}
Die Arbeiten in dieser IPA werden wie ein normales Feature behandelt. 
Das bedeutet, der Code wird sich erst mindestens einem Code Review unter-
ziehen müssen und wenn nötig verbessert werden. Der Betrieb muss letztlich nicht
unbedingt angepasst werden und ist in der Dokumentation auf GitHub beschrieben. 
Jedoch müssen vor dem Merge in den produktiven
Branch des Cores noch einige Arbeiten erledigt werden, welche innerhalb dieser IPA
nicht beachtet wurden.


\section{Weitere Arbeiten}
Beim Filterkriterium ``Tags'' ist es momentan möglich sowohl bei dem Eingabefeld zum Ausschliessen wie auch beim Eingabefeld zum Einschliessen
der Tags das gleiche Tag einzugeben. Dies ist ein logischer Fehler, der jedoch noch auf die alte Grundfunktionalität zurückzuführen ist. Bevor das IPA Feature
gemerged wird sollte für die Behebung dieses Problems etwas Zeit investiert werden.

Während der Umsetzung kam von Daniel Illi der Vorschlag, man könnte für die einzelnen Filterkriterien eine Validierung für deren Werte einfügen. In diesem Fall,
müsste es, wie zu Beginn im Mockup angedacht, möglich sein ein einzelnes Filterkriterium speicher zu können. Bei Fehlschlagen der Validierung würde dann ein entsprechender 
Fehler auf der Benutzerschnittstelle angezeigt werden. Diese Anpassung könnte vor einem Review oder als separates Feature zur Filterung implementiert werden.

Ein wichtiger Aspekt ist zuletzt das Durchtesten mit allen Wagons von Hitobito. Bei vielen Anpassungen mit Core, sollte die Funktionalität in den Wagons mit manuellen sowie
automatisierten Tests sichergestellt werden. Am Besten wird dies vor dem Review durch einen anderen Entwickler durchgeführt, um die Aufkommenden Fehler direkt selber beheben zu können und so
Zeit im Review zu sparen.

\chapter{Abschluss Sprint Finalisierung}
An dieser Stelle endet der erste Sprint welcher sich mit der Finalisierung der
Arbeit befasste. Der Sprint dauerte vom 19.03.2025 bis am 21.03.2025.

\begin{figure}[h]
   \centering
   \fbox{\includegraphics[width=1\textwidth,]{sprint3_backlog.png}}
   \caption{Backlog nach Sprint Finalisierung}
\end{figure}

In diesem Sprint konnten alle User Stories umgesetzt werden. Die letzte User Story ``Delivery and last changes'' wird bis zum 
Ende und der Abgabe der Arbeit in der Spalte ``In-Progress'' bleiben. Wurde die Arbeit abgegeben und signiert, kann diese User Story
...
\begin{figure}[h]
   \centering
   \fbox{\includegraphics[width=0.8\textwidth,]{sprint3_burnup.png}}
   \caption{Burn up nach Sprint 3}
\end{figure}

Anhand des Burn ups sieht man, dass die meisten User Stories bis zum Sprintabschluss abgeschlossen werden konnten. Mit dem heutigen Tag,
wurden alle User Stories (ausgenommen der erwähnten) abgeschlossen.
...
\begin{figure}[h]
   \centering
   \fbox{\includegraphics[width=0.8\textwidth,]{sprint3_chart.png}}
   \caption{Charts nach Sprint 3}
\end{figure}

Insgesamt konnten in dieser IPA 25 User Stories abgeschlossen werden.